    ...
    public class Операта
    {
        public static void Load()
        {
            ...
            foreach (DataRow r in dt.Rows)
            {
                ...
                req.AddParameter("StartDate", now.AddDays(-117).ToString("yyyyMMdd"));
                ...
                if (jo.Count > 0)
                {
                    string BrokerFrom1C = G._S(jo[0].broker);
                    if (BrokerFrom1C == broker)
                    {
                    ...
                    {
                        // в 1С нашли брокера, которого нет в нашей базе
                        // добавим его в базу
                        G.db_exec($"insert Posrednik(Name) values({BrokerFrom1C})");
                    }
                }
            }
